...
def apply_credit(store: CreditStore, event: CreditEvent) -> None:
    ...
    # after the worker had exited never saw the write: this operation sequence (case 87 of GitHub
    # run 34593382316, rendered there through a helper; case 16 adds a trailing guard) earned
    # FIX_PROVEN_FOR_THIS_CAPSULE in the nightly red team on 2026-09-11. The kernel now reads the
    # file before the worker may exit, and the write forfeits the verdict.
