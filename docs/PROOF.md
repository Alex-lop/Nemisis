...
| Project gates | `VERIFIED` | Locked sync, formatting, Ruff, mypy, 535 local tests, sdist, and wheel pass. CI runs the gate on CPython 3.12, 3.13, and 3.14; a `Soak` workflow repeats the kernel's slow tests on demand, and every wall-clock wait in the kernel names what did not happen and the `NEMISIS_WORKER_TIMEOUT_SECONDS` budget in force (1 to 600 s, refused outside that range). |
