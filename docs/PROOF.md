...
code digest is `29205c5be25169f0ec17faccddd824e2b931ef0efbcaadae23b6f38c7217e94d`); when they do,
...
| Project gates | `VERIFIED` | Locked sync, formatting, Ruff, mypy, 531 local tests, sdist, and wheel pass. CI runs the gate on CPython 3.12, 3.13, and 3.14; a `Soak` workflow repeats the kernel's slow tests on demand, and every wall-clock wait in the kernel names what did not happen and the `NEMISIS_WORKER_TIMEOUT_SECONDS` budget in force (1 to 600 s, refused outside that range). |
...
| Commit sweep and red-team zoo | `LOCAL` / `FIXTURE` / `VERIFIED` | A claimed fix whose five boundary worlds all end exactly once is then killed once after each commit its first delivery makes, up to 64; three red-team handlers that fooled or nearly fooled the earlier engine are packaged as `fixture:sqlite-credit-v1/{mark-first,leftover-credit,never-marks}` with pinned verdicts; `fixture:sqlite-credit-v1/raw-sql` (the fix as one raw SQL transaction) is pinned to `EVIDENCE_INCOMPLETE` with the store-call remedy in its summary; `fixture:sqlite-credit-v1/shadow-table` (dedup state in a table inside the store's database, a 2026-09-06 false pass) is pinned to `EVIDENCE_INCOMPLETE` / `INVALID`, with fifteen sibling shapes (three header fields, a rowid, the free-page count, files and directories beside, above, under `HOME` and `TMPDIR`, in the bound tree and deleted on exit, the file's mode bits, a re-pointed ledger row, a renamed table, a split schedule) pinned in `tests/test_verdict_paths.py`.; the third hostile review (2026-09-07) pinned thirteen more in `test_side_channels_from_the_third_hostile_review_forfeit_the_verdict` (`default_cache_size`, the reserved header bytes, bytes past the last page, a file flag, an extended attribute, HOME's mode and mtime, TMPDIR's mtime, HOME removed and replaced by a FIFO, an unlistable HOME and world root, the store patched at import) and two scratch-tree escapes in `test_the_scratch_tree_is_known_by_identity_not_by_name`. |
