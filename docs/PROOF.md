...
code digest is `c7253a7f74e97dcc4d9631007e59faad1fe28f61efcdb9437da28e95f82ea31a`); when they do,
...
| Project gates | `VERIFIED` | Locked sync, formatting, Ruff, mypy, 560 local tests, sdist, and wheel pass. CI runs the gate on CPython 3.12, 3.13, and 3.14; a `Soak` workflow repeats the kernel's slow tests on demand, and every wall-clock wait in the kernel names what did not happen; the message and exit waits name the `NEMISIS_WORKER_TIMEOUT_SECONDS` budget in force (1 to 600 s, refused outside that range), and the reap and cleanup waits after a kill have fixed budgets of one to three seconds. |
