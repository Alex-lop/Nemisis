...
was executed from clean source `c339aa90f32ec0874b03d51ff8432505596694b2` on 2026-09-07 at engine
`29205c5be25169f0ec17faccddd824e2b931ef0efbcaadae23b6f38c7217e94d` and published by the commit
that follows it on `overnight3/morning`. Later commits may change engine bytes (the current engine
code digest is `1afdb6ee88585074eaeda65c50223c18fb73c4b67710affb247ff8e55a51bb15`); when they do,
...
| Repro Capsule | `VERIFIED` | Capsule `190d62ea98c2884eaeb6f8f30f38bdf0c733e40d186b8cfab8896a691696c0f6`, engine digest `29205c5b…`, exact event/environment/seed/tree identities. |
...
| Measured benchmark | `LOCAL` / `FIXTURE` | Result `7d0672e42f30a30ee7a8cba2c52970e7fffbc9e154ea70b8aad21e26604d5540`; strict schema/digest validation passes. A second, independent measurement on GitHub's Linux ([run 34092142968](https://github.com/Alex-lop/Nemisis/actions/runs/34092142968), engine `228430389f…`, before tonight's engine fixes) reproduced the engine and event digests byte-for-byte and the same verdict, with the environment-bound capsule and result digests different as designed. |
...
| Published wheel (`v0.2.0` on PyPI) | `VERIFIED` | Tag `v0.2.0` at `e1aeae0`; `Release` run [34821573278](https://github.com/Alex-lop/Nemisis/actions/runs/34821573278) green on `build`, `github-release`, and `pypi` (Trusted Publisher, no token); `uv tool install nemisis==0.2.0` into a clean tool directory installed 0.2.0 with this tree's engine code digest, `doctor --mode local` READY, `atomic` `FIX_PROVEN_FOR_THIS_CAPSULE`, `tail-bytes` `EVIDENCE_INCOMPLETE`. |
| Project gates | `VERIFIED` | Locked sync, formatting, Ruff, mypy, 621 local tests, sdist, and wheel pass. CI runs the gate on CPython 3.12, 3.13, and 3.14; a `Soak` workflow repeats the kernel's slow tests on demand, and every wall-clock wait in the kernel names what did not happen; the message, reap, and exit waits use the `NEMISIS_WORKER_TIMEOUT_SECONDS` budget in force (1 to 600 s, refused outside that range) and the message and exit waits name it, and the cleanup waits after a kill have fixed budgets of one to three seconds. |
| GitHub composite Action | `VERIFIED_WITH_BOUNDARY` | Exact-SHA CI executes `uses: ./`, expected candidate rejection, artifact validation, installed-wheel smoke, and corrected replay. The copyable workflow pins the reviewed action commit named in [STATUS.md](STATUS.md); `tests/test_docs_identity.py` holds that pin to what `main` runs, and `pin-bump.yml` moves it (its first run is recorded in the report, not here, until it has one). Not exercised by CI: remote-action download, real upload transfer, and the action's Git-ref branch (resolving `base` to a commit SHA and reading the base-owned `.nemisis/config.json`), which every real pull request takes; that branch is covered only by the Python-level Git materialization tests. |
...
| Commit sweep and red-team zoo | `LOCAL` / `FIXTURE` / `VERIFIED` | A claimed fix whose five boundary worlds all end exactly once is then killed once after each commit its first delivery makes, up to 64; three red-team handlers that fooled or nearly fooled the earlier engine are packaged as `fixture:sqlite-credit-v1/{mark-first,leftover-credit,never-marks}` with pinned verdicts; `fixture:sqlite-credit-v1/raw-sql` (the fix as one raw SQL transaction) is pinned to `EVIDENCE_INCOMPLETE` with the store-call remedy in its summary; `fixture:sqlite-credit-v1/shadow-table` (dedup state in a table inside the store's database, a 2026-09-06 false pass) is pinned to `EVIDENCE_INCOMPLETE` / `INVALID`, and `fixture:sqlite-credit-v1/tail-bytes` (bytes past the last page after the last commit, the nightly's false pass of 2026-09-11) is pinned to the same, with fifteen sibling shapes (three header fields, a rowid, the free-page count, files and directories beside, above, under `HOME` and `TMPDIR`, in the bound tree and deleted on exit, the file's mode bits, a re-pointed ledger row, a renamed table, a split schedule) pinned in `tests/test_verdict_paths.py`.; the third hostile review (2026-09-07) pinned thirteen more in `test_side_channels_from_the_third_hostile_review_forfeit_the_verdict` (`default_cache_size`, the reserved header bytes, bytes past the last page written before a commit, a file flag, an extended attribute, HOME's mode and mtime, TMPDIR's mtime, HOME removed and replaced by a FIFO, an unlistable HOME and world root, the store patched at import) and two scratch-tree escapes in `test_the_scratch_tree_is_known_by_identity_not_by_name`. |
...
| Hosted URL and demo video | `PARTIAL` | The landing page and the evidence viewer are live at <https://alex-lop.github.io/Nemisis/>, served from the repository root by GitHub Pages' branch source. A 30-second `vhs` terminal recording of a real local run (`crashcheck-demo.gif`) and terminal stills are committed; no narrated video and no provider run is claimed. |
| Adversarial generator (`nemisis redteam`) | `LOCAL` / `VERIFIED` | A grammar over store operations and the writes around the store that hostile reviews wrote by hand (files beside, above, under `HOME` and `TMPDIR`, a file tidied away, a raw SQL effect, a table, a pragma, a re-pointed row, a swallowed second marker, a retry loop, a helper function, a world-detection attempt) renders handlers in either scenario's vocabulary; an oracle computed from the operation sequence alone, which knows when the kernel probes and when it scans, names the verdict each must earn; `tests/test_redteam.py` pins the oracle on fifty-nine shapes, the packaged zoo among them, runs ten fixed-seed credit handlers, the nine nightly shapes, and five inventory shapes with zero disagreements; `.github/workflows/nightly.yml` runs three hundred per scenario, and its first GitHub run ([34086693284](https://github.com/Alex-lop/Nemisis/actions/runs/34086693284)) had zero disagreements; its next five failing runs (2026-09-08 to 2026-09-13, seeds equal to the run ids) reported ten cases (twenty lines, each case in both scenarios), every one a handler appending bytes past the database's last page after its last store commit, on which the engine issued verdicts and, in run 34593382316, `FIX_PROVEN_FOR_THIS_CAPSULE`, because it read the file only after the worker's exit had truncated it; the engine now reads before the worker may exit, the shape ships as `tail-bytes`, and the nine shapes are pinned in `tests/test_redteam.py`. A load-induced refusal counts as `unknown`, never as agreement. The generator is not an engine resource. |
...
| Judge's first five minutes | `VERIFIED` | `init` refuses a target the base tree cannot bind, in the sentence `check` would print plus the one change that makes it bind; every anchor refusal, the no-crash control, the corrected control, and the eighteen coherence clauses name what happened and the remedy where one exists (`tests/test_crashcheck.py`, `tests/test_verdict_paths.py`). `nemisis --version`, a `Dockerfile` (tested: doctor READY and the zoo verdict inside the image), a `CHANGELOG.md`, and a tag-driven release workflow with PyPI trusted publishing staged; the PyPI project needs the owner's pending-publisher registration. |
...
- [run manifest](assets/crashcheck-hero/runs/local-20260907T070923-1bdaf841/manifest.json)
- [full receipt](assets/crashcheck-hero/runs/local-20260907T070923-1bdaf841/report.html)
- [capsule](assets/crashcheck-hero/repros/double-credit/190d62ea98c2884eaeb6f8f30f38bdf0c733e40d186b8cfab8896a691696c0f6/capsule.json)
