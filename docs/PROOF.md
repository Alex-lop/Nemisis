...
code digest is `e1fdffc69eb40cdff5137a027cf7fcb1f6e5a2080259bc37e9f1611726379c70`); when they do,
...
| Project gates | `VERIFIED` | Locked sync, formatting, Ruff, mypy, 421 local tests, sdist, and wheel pass. |
...
| Adversarial generator (`nemisis redteam`) | `LOCAL` / `VERIFIED` | A grammar over store operations renders handlers; an oracle computed from the operation sequence alone names the verdict each must earn; `tests/test_redteam.py` pins the oracle on the whole zoo and runs ten fixed-seed generated handlers with zero disagreements; `.github/workflows/nightly.yml` runs three hundred. The generator is not an engine resource. |
